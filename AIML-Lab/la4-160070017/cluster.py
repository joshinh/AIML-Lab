	...
	for i in range(len(data)):
		...
		for j in range(len(centroids)):
			...
			if d_min == -1:
				...
				d_min = d
				d_index = j
		assign_list.append(d_index)

	#Compute the new centriods
	for i in range(len(centroids)):
		count = 0
		temp_new_centroids = (0.0,) * len(data[0])
		for j in range(len(assign_list)):
			if assign_list[j] == i:
				temp_new_centroids = tuple(map(lambda x,y: x + y, temp_new_centroids, data[j]))
				count += 1
		if count == 0:
			new_centroids.append(centroids[i])
		else:
			temp_new_centroids = tuple(map(lambda x: x/count, temp_new_centroids))
			new_centroids.append(temp_new_centroids)
...
def iteration_many(data, centroids, distance, maxiter, algorithm, epsilon=1e-1):
	...
	for i in range(maxiter):
		new_centroids = iteration_one(data,all_centroids[-1],distance_euclidean,algorithm)
		all_centroids.append(new_centroids)
		if hasconverged(all_centroids[-1],all_centroids[-2]):
			break
...
def initialization_kmeansplusplus(data, distance, k):
	...
	#Select the first cluster point uniformly at random
	first_index = random.sample(range(len(data)),1)[0]
	centroids.append(data[first_index])
	data_index_list.append(first_index)

	#Select the next k-1 centroids
	for p in range(k-1):

		#We have in D the distance between x_i and the nearest center that has already been chosen
		#D_index is a list used to store the corresponding indices i of the data points
		D = []
		D_index = []
		for i in range(len(data)):
			d_min = -1
			if i not in data_index_list:
				for j in range(len(data_index_list)):
					d_curr = distance(data[i], data[data_index_list[j]])
					if d_min == -1 or d_curr < d_min:
						d_min = d_curr
				D.append(d_min * 1.0)
				D_index.append(i)
		...
		#The below computes the probability as given in question
		D_total = sum(D)
		D_prob = [D[i]/D_total for i in range(len(D))] 

		#Uniformly select a point between 0 and 1
		ru = random.uniform(0,1)

		#We compute the CDF of each of the remaining data points according to the probability above
		#Point i is chosen if ru is between CDF(i-1) and CDF(i) - This ensures that prob ith point is chosen
		#is equal to the difference between CDF(i) and CDF(i-1) which by definition is the required prob.
		D_now = 0
		...
		#i is the index using CDF sampling strategy
		centroids.append(data[D_index[i]])
		data_index_list.append(D_index[i])
...
def kmedians_iteration_one(data, centroids, distance):
	...
	#At this stage we have assigned each data point to its closest centriod

	#We now recalculate the centriods by copmuting the medians
	for i in range(len(centroids)):
		count = 0
		temp_new_centroids = []
		for j in range(len(assign_list)):
			if assign_list[j] == i:
				temp_new_centroids.append(data[j])
				count += 1
		if count == 0:
			new_centroids.append(centroids[i])
		else:
			new_zipped = zip(*temp_new_centroids)
			#new zipped is list of tuples where number of tuples is number of dimesions and number of elements
			#in each tuple is number of points assigned to that cluster
			list_zipped = [list(p) for p in new_zipped]
			[p.sort() for p in list_zipped]
			temp_new_centroids = tuple([compute_median(x) for x in list_zipped]) 
			new_centroids.append(temp_new_centroids)
...
def performance_L1(data, centroids, distance):
	...
	for i in range(len(data)):
		...
		d_index = -1
		for j in range(len(centroids)):
			d = distance_manhattan(data[i], centroids[j])
			if d_min == -1:
				d_min = d
				d_index = j
			elif d < d_min:
