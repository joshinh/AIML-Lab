...
class SudokuSearchProblem:
    ...
    def getSuccessors(self, state):
        ...
        for k,v in state.iteritems():
            if min_value == -1 and len(v) > 1:
                min_value = len(v)
                min_key = k
            if len(v) > 1 and len(v) < min_value:
                min_value = len(v)
                min_key = k
        temp_actions = [(min_key, d) for d in state[min_key]]

        for a in temp_actions:
            temp_state = assign(state.copy(), a[0], a[1])
            if temp_state == False:
                continue
            successors.append((temp_state, a, 1))
        return successors
...
if __name__ == '__main__':
    ...
    prob = SudokuSearchProblem(parse_grid(prob1))
    state = prob.getStartState()
    # print(prob.getSuccessors(state))
    # print(state)