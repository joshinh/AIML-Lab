...
def generate2optNeighbours(tour):
    ...
    "*** YOUR CODE HERE ***"
    #First reverse all substrings in between
    n = len(tour)
    for i in range(1,n-2):
        for j in range(i+2,n):
            tour_new = tour.copy()
            tour_new[i:j] = tour[i:j][::-1]
            all_possible_neighbours.append(tour_new)

    #Add the remaining tours corresponding to edge first-last
    tour_shift = tour.copy()
    tour_shift[0] = tour[-1]
    tour_shift[1:] = tour[:-1]

    for i in range(3,n):
        tour_new = tour_shift.copy()
        tour_new[1:i] = tour_shift[1:i][::-1]
        all_possible_neighbours.append(tour_new)  
...
def generate3optNeighbours(tour):
    ...
    "*** YOUR CODE HERE ***"
    #First generate all neighbours where edge reversals does not include n to 1 edge

    n = len(tour)
    for i in range(1,n-4):
        for j in range(i+1,n-3):
            for k in range(j+2,n-1):
                tour_new_1 = tour.copy()
                tour_new_2 = tour.copy()
                tour_new_3 = tour.copy()
                tour_new_4 = tour.copy()
                
                tour_new_1[i:j+1] = tour_new_1[i:j+1][::-1]
                tour_new_1[j+1:k+1] = tour_new_1[j+1:k+1][::-1]

                tour_new_2[i:k+1] = tour_new_2[i:k+1][::-1]
                tour_new_2[i:j+1] = tour_new_2[i:j+1][::-1]
                tour_new_2[j+1:k+1] = tour_new_2[j+1:k+1][::-1]

                tour_new_3[i:k+1] = tour_new_3[i:k+1][::-1]
                tour_new_3[i:j+1] = tour_new_3[i:j+1][::-1]

                tour_new_4[i:k+1] = tour_new_4[i:k+1][::-1]
                tour_new_4[j+1:k+1] = tour_new_4[j+1:k+1][::-1]

                all_possible_neighbours.append(tour_new_1)
                all_possible_neighbours.append(tour_new_2)
                all_possible_neighbours.append(tour_new_3)
                all_possible_neighbours.append(tour_new_4)

    #Add the rneighbours corresponding the reversals including n to 1 edge
    ...
    for j in range(i+1,n-3):
        for k in range(j+2,n-1):
            tour_new_1 = tour_shift.copy()
            tour_new_2 = tour_shift.copy()
            tour_new_3 = tour_shift.copy()
            tour_new_4 = tour_shift.copy()
...
def hillClimbFull(initial_tour, getNeighbours):
    ...
    def find_mintour(tour_inp):
        neighbours = getNeighbours(tour_inp)
        tour_costs = [getTourLength(x) for x in neighbours]
        min_index = argmin(tour_costs)
        return neighbours[min_index], tour_costs[min_index]

    curr_tour = initial_tour
    curr_cost = getTourLength(curr_tour)
    minTour = initial_tour
    while True:
        next_tour, next_cost = find_mintour(curr_tour)
        if next_cost >= curr_cost:
            break
        curr_tour = next_tour
        curr_cost = getTourLength(curr_tour)
        minTour = curr_tour
        tourLengthList.append(next_cost)
...
def nearestNeighbourTour(initial_city):
    ...
    "*** YOUR CODE HERE ***"
    curr_city = nodeDict[initial_city]
    visited = [initial_city]
    tour.append(curr_city.i)
    for _ in range(cities-1):
        mindist = -1
        for _,j in nodeDict.items():
            if j.i not in visited:
                if mindist == -1 or mindist > getDistance(curr_city, j):
                    mindist = getDistance(curr_city, j)
                    minnode = j
        visited.append(minnode.i)
        tour.append(minnode.i)
        curr_city = minnode
...
def eucledianTour(initial_city):
    ...
    for i in range(1,cities):
        for j in range(i+1,cities+1):
            x = nodeDict[i].i
            y = nodeDict[j].i
            dist = getDistance(nodeDict[i], nodeDict[j])
            edgeList.append([x,y,dist])
...
def finalOrder(mst, initial_city):
    ...
    init_possible = find_correct_edge(mst, curr_city)
    for i in init_possible:
        s.push(i)
        mst.remove(i)
    ...
    while True:
        if s.isEmpty():
            break
        curr_edge = s.pop()
        curr_node = fin_order[-1]
        if curr_edge[0] == curr_node:
            fin_order.append(curr_edge[1])
            curr_node = curr_edge[1]
        elif curr_edge[1] == curr_node:
            fin_order.append(curr_edge[0])
            curr_node = curr_edge[0]
        elif curr_edge[0] in fin_order:
            fin_order.append(curr_edge[1])
            curr_node = curr_edge[1]
        elif curr_edge[1] in fin_order:
            fin_order.append(curr_edge[0])
            curr_node = curr_edge[0]
        else:
            print("Something is wrong")
        
        curr_poss = find_correct_edge(mst,curr_node)
        for i in curr_poss:
            mst.remove(i)
            s.push(i)
